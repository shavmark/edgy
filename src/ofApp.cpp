...
void ListenThread::threadedFunction() {
	...
	if (checkResult(hr) == S_OK) {
		ofFile file;
		if (file.doesFileExist("voice.grxml")) {
			hr = cpGrammar->LoadCmdFromFile(L"data\\voice.grxml", SPLO_STATIC);
		}
	}
	...
	while (fContinue && checkResult(hr) == S_OK) {
...
HRESULT ListenThread::checkResult(const HRESULT& result)
{
	if (result == S_OK) {
		return S_OK;
	}
	...
	default:
		message = "Unknown com error: " + std::to_string(result);
	}
